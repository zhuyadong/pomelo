...
export class Server {
    private opts: any;
    private globalFilterService:FilterService;
	...
	globalHandle(msg, session, cb) {
		...
		if (!routeRecord) {
			utils.invokeCallback(
				cb,
				new Error("meet unknown route message %j", msg.route)
			);
			return;
		}
		...
		var dispatch = function(err, resp, opts) {
			if (err) {
				handleError(true, self, err, msg, session, resp, opts, function(
					err,
					resp,
					opts
				) {
			...
			if (self.app.getServerType() !== routeRecord.serverType) {
				doForward(self.app, msg, session, routeRecord, function(
					err,
					resp,
					opts
				) {
				...
				doHandle(self, msg, session, routeRecord, function(
					err,
					resp,
					opts
				) {
...
var loadCrons = function(server, app) {
	...
	if (!fs.existsSync(p)) {
		p = path.join(
			...
			env,
			path.basename(Constants.FILEPATH.CRON)
		);
		if (!fs.existsSync(p)) {
			return;
		}
...
var doHandle = function(server, msg, session, routeRecord, cb) {
	...
	var handle = function(err, resp, opts) {
		if (err) {
			...
			handleError(false, self, err, msg, session, resp, opts, function(
				err,
				resp,
				opts
			) {
		...
		self.handlerService.handle(routeRecord, msg, session, function(
			err,
			resp,
			opts
		) {
